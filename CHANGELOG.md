...
### Added — 2026-06-23 (S1.1–S1.4)
- 搜索优化：原生混合查询 — Qdrant Query API（prefetch + RRF fusion），稠密向量 + BM25 稀疏向量并行搜索
- 搜索优化：Grouping API — 按 `doc_id` 分组去重，每文档只保留最佳 chunk
- 性能优化：int8 标量量化（`quantization_config.scalar.type=int8`），内存降低约 75%
- 集合配置：重建 `athanor_v1` 集合（未命名稠密 + BM25 稀疏 + int8 量化）
- 移除：旧 `hybrid_search()` 和 `_keyword_search()` 函数（被原生 query API 替代）

### Fixed — 2026-06-23
- 向量格式：`ingest_pipeline.py` 中稠密向量与稀疏向量从混合同一字段改为独立字段（`"vector"` + `"sparse_vectors"`）
- 摄入管线：`_step_build_payloads` 补充 `sparse_vectors` 传参

### Added — 2026-06-23 (S2)
- 搜索优化：混合检索（向量 + BM25 关键词并行）
- 搜索优化：重排序（bge-reranker-v2-m3 本地模型，Ollama 运行）
