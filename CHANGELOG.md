...
- **R1 置信度阈值可配置**：引擎配置「⚙️ 系统」标签页增加高/低阈值滑动条，存 `.env`；摄入页动态读取阈值
- **R2 待审核队列 UI**：知识中枢页面新增「📋 待审核」标签页，显示置信度进度条，支持通过（取消标记）/ 丢弃（删除文档）
- **R3 死信队列 UI**：知识中枢页面新增「🗑️ 死信队列」标签页，支持三种处理方式：手动修正分类后重新摄入 / 重新上传文件替换 / 永久删除
...
- `list_documents()` 增强：新增返回 `overall_confidence`、`needs_review`、`content_preview`、`source_path` 字段
